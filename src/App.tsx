...
// --- Touchup Result Card Component (From HTML Logic) ---
...
const AdminPanel = ({ onBack }: { onBack: () => void }) => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-20 fade-in">
        ...
        {/* Touchup Search Modal - Connected to GAS API */}
  ...
  // Helper state for search modal
  const [searching, setSearching] = useState(false);
  const [results, setResults] = useState<TouchupRecord[]>([]);
  const [msg, setMsg] = useState({ type: '', text: '' });

  // Effect to trigger search when modal opens with a query
  useEffect(() => {
