...
export interface TouchupRecord {
  name: string; phone: string; lastDate: string; service: string; specialReminder?: string;
  eyebrowDiscountDate?: string; currentPriceInfo?: { price: number; dateRange: string; };
  touchUpRanges?: { dateRange: string; price: number; }[];
}
...
// --- Touchup Result Card Component ---
const TouchupResultCard = ({ r }: { r: TouchupRecord }) => {
    ...
    const maskPhone = (p: string) => {
        ...
        return s.length < 8 ? s : s.substring(0, 4) + '***' + s.substring(s.length - 3);
    };
    
    const checkIsExpired = (d?: string) => {
        if (!d) return false;
        return new Date() > new Date(d);
    };

    // Current price logic
    const info = r.currentPriceInfo;
    const useOrange = r.eyebrowDiscountDate && !checkIsExpired(r.eyebrowDiscountDate);
    const priceDisplay = info ? (info.price === 0 ? '免費' : (info.price ? `NT$ ${info.price}` : '洽詢')) : '';
    const bgClass = info?.price === 0 ? 'bg-green-600' : (useOrange ? 'bg-orange-500' : 'bg-[#D6BFA8]'); 
    
    return (
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 mb-4 animate-fadeIn text-left">
            <div className="flex justify-between items-start mb-4">
                <div>
                    <h3 className="text-xl font-bold text-[#5d4037]">{r.name}</h3>
                    <span className="text-sm text-gray-400">{maskPhone(r.phone)}</span>
                </div>
            </div>
            
            <div className="flex gap-2 mb-4">
                <div className="flex-1 bg-[#F9F8F6] p-3 rounded-xl border border-gray-100 text-center">
                    <span className="block text-xs text-gray-400 mb-1">上次日期</span>
                    <span className="block font-bold text-[#5d4037]">{r.lastDate || '-'}</span>
                </div>
                <div className="flex-1 bg-[#F9F8F6] p-3 rounded-xl border border-gray-100 text-center">
                    <span className="block text-xs text-gray-400 mb-1">上次項目</span>
                    <span className="block font-bold text-[#8d6e63]">{r.service}</span>
                </div>
            </div>

            {r.specialReminder && (
                <div className="bg-yellow-50 text-yellow-800 p-3 rounded-xl text-xs mb-4 font-medium flex gap-2">
                    <span>💡</span><span>{r.specialReminder}</span>
                </div>
            )}

            {info && (
                <div className={`border rounded-xl p-3 flex justify-between items-center mb-4 ${info.price === 0 ? 'border-green-200 bg-green-50' : 'border-[#EAE0D5] bg-[#FFFBF9]'}`}>
                    <div>
                        <div className="text-xs font-bold text-gray-400 mb-1">目前補色價格</div>
                        <div className="text-sm font-bold text-[#5d4037]">{(info.dateRange || '').replace(/【.*?】/g, '')}</div>
                    </div>
                    <span className={`px-3 py-1 text-white text-sm font-bold rounded-lg shadow-sm ${bgClass}`}>
                        {priceDisplay}
                    </span>
                </div>
            )}
            ...
            <div>
                <button 
                    ...
                    className="w-full py-2 bg-gray-50 text-gray-500 rounded-lg text-xs flex items-center justify-center gap-1 hover:bg-gray-100 transition-colors">
                    {isExpanded ? '收起價目表' : '查看完整價目表'}
                    <Icon name={isExpanded ? 'up' : 'down'} size={12} />
                </button>
                
                {isExpanded && (
                    <div className="mt-2 bg-gray-50 rounded-xl p-3 space-y-2 animate-fadeIn">
                        {r.touchUpRanges?.map((t, idx) => (
                            <div key={idx} className="flex justify-between text-sm border-b border-gray-200 last:border-0 pb-2 last:pb-0">
                                <span className="text-gray-600">{t.dateRange}</span>
                                <span className="font-bold text-[#5d4037]">{t.price === 0 ? '免費' : `$${t.price}`}</span>
                            </div>
                        ))}
                    </div>
                )}
...
const AdminPanel = ({ onBack }: { onBack: () => void }) => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-20 fade-in">
                ...
                <div className="space-y-6">
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="bg-blue-50 p-3 rounded text-xs text-blue-700 mb-2">
                            可用變數：{'{{name}}'} 姓名, {'{{date}}'} 日期, {'{{time}}'} 時間, {'{{location}}'} 地點, {'{{service}}'} 服務
                        </div>
        ...
        <Modal title={actionType === 'confirm' ? '確認預約' : actionType === 'verify' ? '確認收款' : actionType === 'no-deposit' ? '無需訂金確認' : '取消預約'} 
               isOpen={!!actionBooking} onClose={() => { setActionBooking(null); setActionType(null); }}>
             <div className="space-y-4">
                 <p className="text-sm text-gray-500">
                     {actionType === 'confirm' ? '將變更為「已確認」並標記「已收款」。' : 
                      actionType === 'no-deposit' ? '將訂金設為 $0，標記「已收款」並確認預約。' : 
                      '執行後將更新狀態，您可複製以下訊息傳送給客人：'}
                 </p>
                 <textarea 
                 ...
                 />
                 <div className="flex gap-2">
                     <Button variant="outline" className="flex-1" onClick={() => copyToClipboard(actionMessage)}>
                         <Icon name="copy"/> 複製訊息
                     </Button>
        ...
        <Modal title="快速新增預約" isOpen={isManualAddOpen} onClose={() => setIsManualAddOpen(false)}>
             <div className="space-y-4">
                 <div>
                     ...
                     <div className="flex gap-2">{LOCATIONS.map(l => (<button key={l.id} onClick={() => setManualBooking({...manualBooking, locationId: l.id})} className={`flex-1 py-2 text-sm rounded border ${manualBooking.locationId === l.id ? 'bg-[#8d6e63] text-white border-[#8d6e63]' : 'bg-white border-gray-200'}`}>{l.name}</button>))}</div>
                 </div>
                 <div className="flex gap-2">
                     <div className="flex-1"><label className="text-xs font-bold text-gray-500 block mb-1">日期</label><input type="date" className="w-full p-2 border rounded" value={manualBooking.date} onChange={e => setManualBooking({...manualBooking, date: e.target.value})} /></div>
                     <div className="flex-1"><label className="text-xs font-bold text-gray-500 block mb-1">時間</label><input type="time" className="w-full p-2 border rounded" value={manualBooking.time} onChange={e => setManualBooking({...manualBooking, time: e.target.value})} /></div>
                 </div>
                 <div><label className="text-xs font-bold text-gray-500 block mb-1">顧客姓名</label><input className="w-full p-2 border rounded" value={manualBooking.name} onChange={e => setManualBooking({...manualBooking, name: e.target.value})} /></div>
...
export default function App() {
  ...
  const [page, setPage] = useState<'home' | 'booking' | 'status' | 'admin-login' | 'admin' | 'touchup'>('home');
  ...
  // Helper state for search modal (Moved up from bottom)
  const [searching, setSearching] = useState(false);
  const [results, setResults] = useState<TouchupRecord[]>([]);
  const [msg, setMsg] = useState({ type: '', text: '' });

  // Effect to trigger search when modal opens with a query (Moved up from bottom)
  useEffect(() => {
      if (touchupQuery && (page === 'home' || page === 'touchup')) {
          setSearching(true);
          setResults([]);
          setMsg({ type: '', text: '' });
          ...
          const runSearch = async () => {
              try {
                  ...
                  if (data.data && data.data.length > 0) {
                      ...
                      setResults(filteredResults as TouchupRecord[]);
                      
                      if (filteredResults.length > 0) {
                          setMsg({ type: 'success', text: `找到 ${filteredResults.length} 筆資料` });
                      } else {
                          setMsg({ type: 'error', text: '查無資料，請確認輸入是否正確' });
                      }
                  ...
                  console.error(err);
                  setMsg({ type: 'error', text: '連線失敗，請稍後再試' });
              } finally {
                  setSearching(false);
              }
          };
  ...
  useEffect(() => {
    ...
    const unsubs = [
      firebaseService.getServices(data => { if(data.length) setServices(data) }),
      firebaseService.getDiscounts(setDiscounts),
      firebaseService.getSettings(setSettings)
    ];
    return () => unsubs.forEach(u => u());
  ...
  useEffect(() => {
    if (location && date) {
        ...
        const unsub = firebaseService.getBookingsByDate(location.id, dStr, (data) => {
  ...
  const totalPrice = useMemo(() => {
    ...
    guests.forEach(g => {
        ...
        const hasBrowFirst = g.services.some(s => s.category === '霧眉' && s.type === '首次');
        const hasLipFirst = g.services.some(s => s.category === '霧唇' && s.type === '首次');
  ...
  const handleServiceSelect = (newSvc: Service) => {
      ...
      const sameCategoryIndex = currentServices.findIndex(s => s.category === newSvc.category);
  ...
  const isTimeSlotTaken = (slot: string, myGuestIndex: number) => {
      ...
      const myDuration = calculateGuestDuration(guests[myGuestIndex]?.services || []);
      ...
      for (const [gIdxStr, t] of Object.entries(guestTimes)) {
          ...
          if (gIdx !== myGuestIndex) {
              ...
              const otherDuration = calculateGuestDuration(guests[gIdx]?.services || []);
              const otherEnd = otherStart + otherDuration;

              if (Math.max(slotStart, otherStart) < Math.min(myEnd, otherEnd)) return true;
          }
      }
      return false;
  };
  ...
  const submitBooking = async () => {
    ...
    const bookingPayloads = guests.map((g, i) => {
        ...
        const gTotal = g.services.reduce((acc,s)=>acc+s.price,0) - totalDiscount;
  ...
  const handleTouchupSearch = () => {
      ...
      // Logic handled in Modal useEffect
  };
            ...
            <div className="space-y-4">
                {LOCATIONS.map(l => (
                    <button key={l.id} onClick={() => { setLocation(l); setPage('booking'); setStep(1); }} className="w-full p-6 bg-white rounded-3xl border border-[#e7e0da] shadow-sm flex flex-col items-center justify-center gap-3 transition-all duration-200 group active:scale-95 hover:shadow-md hover:border-[#d7ccc8]">
                        ...
                        <div className="text-xs text-gray-400 tracking-wider">立即預約</div>
                    </button>
                ))}
            </div>

            <div className="bg-white p-4 rounded-2xl border border-dashed border-[#d7ccc8] mt-4 shadow-sm">
                <label className="text-xs font-bold text-[#8d6e63] mb-2 block text-left">補色價格查詢</label>
                <div className="flex gap-2">
                    <input 
                    ...
                    />
                    <Button onClick={handleTouchupSearch} className="py-2 px-4 h-full text-xs">查詢</Button>
                </div>
            </div>
  ...
  return (
    <div className="min-h-screen pb-28 bg-[#faf9f6]">
        ...
        <div className="max-w-md mx-auto p-4 space-y-6">
            {step === 1 && (
                <div className="fade-in space-y-6">
                    <div className="flex bg-white p-1.5 rounded-xl border border-gray-200 shadow-sm">
                        <button onClick={() => { setIsMulti(false); setGuests([guests[0]]); }} className={`flex-1 py-2 rounded-lg text-sm font-bold transition-all ${!isMulti ? 'bg-[#5d4037] text-white shadow-md' : 'text-gray-400 hover:bg-gray-50'}`}>單人預約</button>
                        <button onClick={() => { setIsMulti(true); if(guests.length < 2) setGuests([...guests, { id: Date.now(), name:'', phone:'', services: [], discount: null }]); }} className={`flex-1 py-2 rounded-lg text-sm font-bold transition-all ${isMulti ? 'bg-[#5d4037] text-white shadow-md' : 'text-gray-400 hover:bg-gray-50'}`}>多人同行</button>
                    </div>
                    ...
                    {guests.map((g, i) => {
                        ...
                        let discountText = '';
                        if (hasBrowFirst && hasLipFirst) discountText = '✨ 組合優惠 (眉+唇)：折抵 $400';
                        else if (hasAnyFirst && hasAnyTouchup) discountText = '✨ 舊客優惠 (首次+補色)：折抵 $200';
                        else if (isMulti && hasAnyFirst) discountText = '✨ 多人同行優惠：折抵 $200';
                        
                        // Duration Calculation
                        const duration = calculateGuestDuration(g.services);
                        const durationText = duration > 0 ? `${Math.floor(duration/60)}小時 ${duration%60 > 0 ? `${duration%60}分` : ''}` : '';

                        return (
                        <div key={g.id} className="relative pl-5 border-l-4 border-[#8d6e63] bg-white p-5 rounded-r-2xl shadow-sm">
                            <div className="flex justify-between items-center mb-4">
                                <span className="bg-[#4e342e] text-white text-xs px-3 py-1 rounded-full font-bold tracking-wide">第 {i + 1} 位</span>
                                {isMulti && i > 0 && <button onClick={() => { const ng = [...guests]; ng.splice(i, 1); setGuests(ng); }} className="text-red-300 hover:text-red-500"><Icon name="trash" /></button>}
                            </div>
                            
                            {g.services.length > 0 && (
                                <div className="space-y-3 mb-4">
                                    {g.services.map(s => (
                                        <div key={s.id} className="bg-[#fffbf9] p-4 rounded-xl border border-[#e7e0da] flex justify-between items-center relative overflow-hidden">
                                            <div className="absolute left-0 top-0 bottom-0 w-1 bg-[#8d6e63]"></div>
                                            <div>
                                                <div className="font-bold text-[#4e342e]">{s.name}</div>
                                                <div className="text-sm text-[#8d6e63] font-bold mt-1">${s.price}</div>
                                            </div>
                                            <button onClick={() => handleGuestUpdate(i, 'services', g.services.filter(srv => srv.id !== s.id))} className="text-gray-300 hover:text-red-400 p-2"><Icon name="trash" size={18} /></button>
                                        </div>
                                    ))}
                                    {durationText && <div className="text-xs text-gray-500 text-right mt-1">預計操作時間: {durationText}</div>}
                                </div>
                            )}

                            <Button variant="outline" className="w-full mb-4 border-dashed" onClick={() => { setCurrentGuestIndex(i); setModalOpen(true); }}>
                            ...
                            {!discountText && !g.services.some(s=>s.type==='補色') && g.services.length > 0 && (
                                <div className="mt-4 pt-4 border-t border-gray-100">
                                    ...
                                    <select className="w-full p-2 bg-[#faf9f6] border border-[#e7e0da] rounded-lg text-sm text-[#5d4037] outline-none"
                                        value={g.discount?.id || ''}
                                        onChange={e => handleGuestUpdate(i, 'discount', discounts.find(d => d.id === e.target.value) || null)}>
                                        <option value="">無折扣 (原價)</option>
                                        {discounts.map(d => <option key={d.id} value={d.id}>{d.name} (-${d.amount})</option>)}
                                    </select>
                                </div>
                            )}
                    ...
                    {isMulti && (
                        <Button variant="primary" className="w-full" onClick={() => setGuests([...guests, { id: Date.now(), name:'', phone:'', services: [], discount: null }])}>
                ...
                <div className="fade-in space-y-6">
                    <Card>
                        <div className="flex justify-between items-center mb-6">
                            <button onClick={() => setDate(new Date(date.getFullYear(), date.getMonth() - 1))} className="p-2 hover:bg-gray-100 rounded-full"><Icon name="chevronLeft" /></button>
                            <span className="font-bold text-lg text-[#5d4037]">{date.getFullYear()}年 {date.getMonth() + 1}月</span>
                            <button onClick={() => setDate(new Date(date.getFullYear(), date.getMonth() + 1))} className="p-2 hover:bg-gray-100 rounded-full"><Icon name="chevronRight" /></button>
                        </div>
                        ...
                        <div className="grid grid-cols-7 gap-2">
                            ...
                            {Array.from({ length: new Date(date.getFullYear(), date.getMonth() + 1, 0).getDate() }).map((_, i) => {
                                ...
                                const isAllowed = settings[location!.id]?.allowedDates?.includes(dStr) ?? true;
                                const isSel = d === date.getDate();
                                const isPast = curr < new Date(new Date().setHours(0,0,0,0));
                                
                                return (
                                    <button key={d} disabled={!isAllowed || isPast} onClick={() => { setDate(curr); setGuestTimes({}); }} 
                                        className={`h-9 w-9 rounded-xl text-sm flex items-center justify-center font-bold transition-all duration-200 
                                        ${isSel ? 'bg-[#8d6e63] text-white shadow-md scale-110' : (!isAllowed || isPast) ? 'text-gray-200 cursor-not-allowed' : 'hover:bg-[#f5f5f5] text-gray-700'}`}>
                                        {d}
                                    </button>
                                );
                            })}
                    ...
                    <div>
                        <h3 className="font-bold text-[#5d4037] mb-3 flex items-center gap-2 text-lg"><Icon name="clock" /> 選擇時段</h3>
                        {guests.length > 1 && <p className="text-xs text-[#8d6e63] mb-3 bg-orange-50 p-2 rounded-lg">⚠️ 同時段需輪流操作，請為每位賓客選擇連續時段 (例如: 11:00, 13:00)</p>}
                        
                        {guests.length > 1 && (
                            <div className="flex gap-2 mb-4 overflow-x-auto no-scrollbar">
                                {guests.map((g, i) => (
                                    <button key={i} onClick={() => setActiveTimeTab(i)} 
                                        className={`px-4 py-2 rounded-full text-sm font-bold whitespace-nowrap transition-all 
                                        ${activeTimeTab === i ? 'bg-[#5d4037] text-white shadow-md' : 'bg-white border border-gray-200 text-gray-500'}`}>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            {(settings[location!.id]?.specialRules?.[date.toISOString().split('T')[0]] || settings[location!.id]?.timeSlots || DEFAULT_SLOTS).map(t => {
                                ...
                                return (
                                    <button key={t} disabled={isTaken && !isSelected} onClick={() => { 
                                        ...
                                        ${isSelected ? 'bg-[#8d6e63] text-white border-[#8d6e63] shadow-md transform scale-[1.02]' : isTaken ? 'bg-gray-100 text-gray-300 border-transparent cursor-not-allowed' : 'bg-white border-transparent shadow-sm text-gray-600 hover:border-[#d7ccc8]'}`}>
                                        {t} {isTaken && !isSelected && <span className="text-[10px] block font-normal opacity-80">已滿 / 時段佔用</span>}
                                    </button>
                                );
                            })}
                        </div>
                        
                        {guestTimes[guests.length > 1 ? activeTimeTab : 0] === '微調時段申請' && (
                            <div className="mt-4 bg-white border border-[#d7ccc8] p-4 rounded-xl fade-in">
                                <label className="text-xs font-bold text-[#8d6e63] mb-2 block">請輸入希望時間</label>
                                <input type="time" className="w-full text-xl font-bold bg-gray-50 p-2 rounded-lg text-center outline-none focus:ring-1 focus:ring-[#8d6e63]" value={customTime} onChange={e => setCustomTime(e.target.value)} />
                            </div>
                        )}
                    </div>
                </div>
                ...
                <div className="fade-in space-y-4">
                    ...
                    {guests.map((g, i) => (
                        <Card key={g.id} className="space-y-4 relative overflow-visible">
                            <div className="absolute -top-3 left-4 bg-[#8d6e63] text-white text-xs px-2 py-1 rounded font-bold">第 {i+1} 位</div>
                            <div className="relative mt-2">
                                <label className="text-xs font-bold text-gray-400 absolute left-3 top-2">姓名</label>
                                <input className="w-full pt-7 pb-2 px-3 bg-[#FAFAFA] rounded-xl border-none font-bold text-[#5d4037] focus:ring-1 focus:ring-[#C4A48C] outline-none" 
                                    placeholder="真實姓名" 
                                    value={g.name} 
                                    onChange={e => handleGuestUpdate(i, 'name', e.target.value)} />
                            </div>
                            <div className="relative">
                                <label className="text-xs font-bold text-gray-400 absolute left-3 top-2">電話</label>
                                <input type="tel" className="w-full pt-7 pb-2 px-3 bg-[#FAFAFA] rounded-xl border-none font-bold text-[#5d4037] focus:ring-1 focus:ring-[#C4A48C] outline-none" 
                    ...
                    <div className="bg-[#FFFBF9] p-6 rounded-2xl border border-[#EBE0D9] space-y-3 shadow-sm">
                        <h3 className="font-bold text-[#5D4037] mb-2 text-lg">匯款資訊</h3>
                        <div className="flex justify-between items-center text-sm text-gray-600">
                            <span>銀行代碼</span>
                            <span className="font-bold text-[#5d4037]">{BANK_INFO.code} ({BANK_INFO.bankName})</span>
                        </div>
                        <div className="flex justify-between items-center text-sm text-gray-600">
                            <span>匯款帳號</span>
                            <div className="flex items-center gap-2">
                                <span className="font-bold text-[#5d4037] tracking-widest">{BANK_INFO.account}</span>
                                <button onClick={()=>copyToClipboard(BANK_INFO.account)} className="text-[#8d6e63] bg-white border border-[#e7e0da] px-2 py-0.5 rounded text-xs">複製</button>
                            </div>
                        </div>
                        <div className="border-t border-dashed border-[#D7CCC8] my-2"></div>
                        <div className="flex justify-between items-end">
                            <span className="text-sm font-bold text-[#8d6e63]">應付訂金</span>
                            <span className="text-red-500 font-bold text-2xl">${totalDeposit}</span>
                        </div>
                        <p className="text-[10px] text-gray-400 text-center mt-2">請於預約後 24 小時內完成匯款並回報</p>
                    </div>
                    ...
                    <label className="flex items-center gap-3 justify-center py-4 cursor-pointer bg-white rounded-xl border border-gray-100 shadow-sm">
                        <input type="checkbox" checked={agreed} onChange={e => setAgreed(e.target.checked)} className="w-5 h-5 accent-[#8D6E63]" />
                        <span className="text-sm font-bold text-[#5d4037]">我已閱讀並同意預約須知</span>
                    </label>
                </div>
            )}
            ...
            {step === 4 && (
                <div className="text-center pt-6 fade-in px-4">
                    ...
                    <h2 className="text-2xl font-bold mb-2 text-[#5d4037]">預約已送出！</h2>
                    <p className="text-gray-500 mb-6 text-sm">
                        感謝您的預約。請<span className="text-red-500 font-bold">截圖</span>下方資訊，<br />
                        並於匯款後點擊下方按鈕回報。
                    </p>

                    <div className="bg-white p-5 rounded-2xl border border-gray-200 shadow-sm mb-6 text-left relative">
                         <div className="absolute top-2 right-2 text-xs bg-gray-100 text-gray-500 px-2 py-1 rounded">請截圖保存</div>
                         <h3 className="font-bold text-[#8d6e63] mb-3 border-b pb-2">匯款資訊</h3>
                         <div className="space-y-2 text-sm">
                             <div className="flex justify-between">
                                 <span className="text-gray-500">銀行</span>
                                 <span className="font-bold">{BANK_INFO.code} {BANK_INFO.bankName}</span>
                             </div>
                             <div className="flex justify-between items-center">
                                 <span className="text-gray-500">帳號</span>
                                 <div className="flex gap-2 items-center">
                                     <span className="font-bold tracking-wider">{BANK_INFO.account}</span>
                                     <button onClick={()=>copyToClipboard(BANK_INFO.account)} className="text-blue-500 text-xs underline">複製</button>
                                 </div>
                             </div>
                             <div className="flex justify-between">
                                 <span className="text-gray-500">金額</span>
                                 <span className="font-bold text-red-500">${totalDeposit}</span>
                             </div>
                         </div>
                    </div>

                    <div className="space-y-3">
                        <Button onClick={() => setPage('status')} variant="primary" className="w-full">
                           前往匯款回報
                        </Button>
                ...
                <div className="flex-1 flex gap-3">
                     ...
                     <Button variant="secondary" className="flex-1 h-full shadow-xl" onClick={() => {
                        ...
                        <div className="flex flex-col items-center leading-none py-1">
                            ...
                            <span className="text-lg">{step === 3 ? '確認送出' : '下一步'}</span>
                        </div>
                    </Button>
                </div>
            </div>
        ...
        <Modal title="選擇服務項目" isOpen={modalOpen} onClose={() => setModalOpen(false)}>
            <ServiceSelection 
                services={services} 
...
const ServiceSelection = ({ services, onSelect, onCancel }: { services: Service[]; onSelect: (s: Service) => void; onCancel: () => void }) => {
  const [stage, setStage] = useState<'main' | 'sub' | 'session' | 'time' | 'confirm'>('main');
  const [mainCat, setMainCat] = useState<string | null>(null);
  const [subCat, setSubCat] = useState<string | null>(null);
  const [session, setSession] = useState<string | null>(null);
  const [isDarkLip, setIsDarkLip] = useState(false);

  const sortedServices = useMemo(() => [...services].sort((a, b) => (a.order || 999) - (b.order || 999)), [services]);
  ...
  if (stage === 'main') return (
    <div className="space-y-4 fade-in">
        <div className="flex justify-between items-center mb-2">
            <p className="text-sm text-[#8d6e63] font-bold tracking-wide">請選擇服務項目：</p>
        </div>
        <div className="grid gap-4">
            {MAIN_CATS.map(c => (
                <button key={c} onClick={() => { setMainCat(c); setStage('sub'); }} className="flex flex-col items-center justify-center p-6 rounded-3xl transition-all duration-300 shadow-sm border border-[#e7e0da] bg-white text-[#5d4037] hover:border-[#d7ccc8] hover:shadow-md hover:-translate-y-1">
                    <div className="text-[#8d6e63] mb-3">
                        {c.includes('眉') ? <Icon name="eye" size={40} /> : <Icon name="smile" size={40} />}
                    </div>
              ...
              {mainCat === '霧唇' && subCat === '首次' && (
                  <div className="mb-6 p-4 bg-white border-2 border-[#d7ccc8] rounded-2xl shadow-sm cursor-pointer hover:bg-[#fff8f6] transition-colors" onClick={() => setIsDarkLip(!isDarkLip)}>
                      <label className="flex items-center gap-4 cursor-pointer pointer-events-none">
                          ...
                          <div>
                              <div className="font-bold text-[#5d4037] text-lg">👄 需要烏唇/淡色處理？</div>
                              <div className="text-sm text-gray-500 mt-1">若唇色較深或暗沈，需先進行淡色處理 (+$1300)</div>
                          </div>
                      </label>
                  </div>
              )}
  ...
  const ranges = sortedServices
      .filter(s => s.category === mainCat && s.type === '補色' && s.session === session)
  ...
  return (
      <div className="space-y-3 fade-in">
          ...
          {ranges.length > 0 ? ranges.map(r => {
             const target = sortedServices.find(s => s.category === mainCat && s.type === '補色' && s.session === session && s.timeRange === r.label);
             return (
              <button key={r.label} onClick={() => target && onSelect(target)} className="w-full p-4 rounded-2xl mb-3 flex justify-between items-center bg-white border border-[#e7e0da] shadow-sm group hover:bg-[#fffbf9]">
                  <span className="font-medium text-lg text-[#5d4037]">{r.label}</span>
          ...
          }) : <div className="text-gray-400 text-center py-8 bg-gray-50 rounded-xl border-2 border-dashed">尚無此時段的報價資料</div>}
      </div>
  );
};
...
const StatusPage = ({ onBack }: { onBack: () => void }) => {
    ...
    const handleSearch = async () => {
        ...
        try {
            ...
            if(valid.length === 0 && data.length > 0) alert('查無有效預約 (可能已過期或取消)');
    ...
    return (
        <div className="min-h-screen bg-[#faf9f6] p-4 fade-in">
            ...
            <div className="bg-[#FFFBF9] p-4 rounded-2xl border border-[#EBE0D9] mb-4 shadow-sm relative">
                ...
                <div className="flex justify-between items-center">
                    <div>
                        <div className="text-sm text-gray-600">{BANK_INFO.code} {BANK_INFO.bankName}</div>
                        <div className="font-bold text-xl text-[#5d4037] tracking-wider">{BANK_INFO.account}</div>
                    </div>
                    <button onClick={() => copyToClipboard(BANK_INFO.account)} className="bg-white border border-[#d7ccc8] text-[#8d6e63] px-3 py-1 rounded-lg text-xs font-bold shadow-sm active:scale-95">
                        複製
                    </button>
                </div>
            </div>
            ...
            <Card className="mb-6">
                <h2 className="font-bold text-xl mb-4 text-[#4e342e]">預約查詢 / 匯款回報</h2>
                <div className="flex gap-2">
                    <input className="flex-1 p-3 border border-[#d7ccc8] rounded-xl outline-none" placeholder="輸入預約電話" value={phone} onChange={e => setPhone(e.target.value)} />
                    <Button onClick={handleSearch} className="w-24">{loading ? <Spinner /> : '查詢'}</Button>
                </div>
            </Card>
            <div className="space-y-4">
                {bookings.map(r => (
                    <Card key={r.id}>
                        <div className="flex justify-between mb-3">
                            <span className="font-bold text-lg text-[#8d6e63]">{r.date} {r.time}</span>
                            <span className={`text-xs px-3 py-1 rounded-full font-bold ${r.status === 'confirmed' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>
                                {r.status === 'confirmed' ? '✅ 成功' : '⏳ 待確認'}
                            </span>
                        </div>
                        <div className="text-base text-gray-700 mb-3">{r.serviceName} {r.guestIndex && `(第${r.guestIndex}位)`}</div>
                        <div className="text-sm font-bold text-[#5d4037] mb-3 bg-[#fdfbf7] p-2 rounded-lg inline-block">訂金：${r.deposit}</div>
                        <div className="border-t border-[#f3f4f6] pt-3 flex justify-between items-center mt-2">
                             <span className="text-sm text-gray-500 font-medium">狀態：{r.paymentStatus === 'verified' ? <span className="text-green-600 font-bold">已入帳</span> : r.paymentStatus === 'reported' ? <span className="text-blue-600 font-bold">審核中</span> : <span className="text-red-500">未支付</span>}</span>
                             {r.paymentStatus === 'unpaid' && (
                                 <Button variant="primary" className="py-1 px-4 text-sm h-8" onClick={() => setReportId(r.id)}>回報</Button>
                             )}
                        </div>
                        {reportId === r.id && (
                            <div className="mt-4 bg-[#fdfbf7] p-4 rounded-xl border border-[#d7ccc8]">
                                ...
                                <div className="flex gap-2">
                                    <input className="flex-1 p-2 border border-[#d7ccc8] rounded-lg text-sm bg-white" placeholder="帳號後五碼" value={last5} onChange={e => setLast5(e.target.value)} />
                                    <Button className="py-1 px-3 text-sm" onClick={handleReport}>送出</Button>
                                    <Button variant="ghost" className="" onClick={() => setReportId(null)}>取消</Button>
                                </div>
                            </div>
                        )}
                    </Card>
            ...
            <div className="mt-10">
                <Button variant="ghost" className="w-full" onClick={onBack}>返回首頁</Button>
            </div>
        </div>
    );
};
