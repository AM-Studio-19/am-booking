...
import { 
  ...
  DEFAULT_SLOTS, BANK_INFO, ADMIN_PIN, MOCK_SERVICES 
} from './constants';
import { 
  Service, Guest, Location, BookingRecord, AppSettings, Discount, Template
} from './types';

// --- Types for Touchup Search ---
interface TouchupRecord {
...
const AdminPanel = ({ onBack }: { onBack: () => void }) => {
  ...
  const handleManualAdd = async () => {
      if(!manualBooking.name || !manualBooking.phone || !manualBooking.serviceId) {
          alert('請填寫完整資訊');
          return;
      }
      ...
      if (showTrash) {
          return (
              <div className="pb-20">
                  ...
                  <Section title="已取消清單" list={targetBookings} />
                  {targetBookings.length === 0 && <div className="text-center text-gray-400 mt-10">垃圾桶是空的</div>}
              </div>
          )
      }
                ...
                <div className="flex gap-2">
                    {tab === 'bookings' && (
                        <button 
                            ...
                            className={`text-xs px-3 py-1 rounded-full border ${showTrash ? 'bg-red-100 text-red-600 border-red-200' : 'bg-gray-50 text-gray-400 border-gray-200'}`}
                            title="顯示已取消的預約"
                        >
                            <span className="mr-1">🗑️</span>垃圾桶
                        </button>
                    )}
            ...
            {tab === 'settings' && (
                <div className="text-center text-gray-400 py-10">
                    <div className="space-y-6 text-left">
                        ...
                        <div className="text-xs text-gray-400 text-center">如需設定特定日期公休，請使用月曆功能</div>
                    </div>
                </div>
            )}
            {tab === 'others' && (
...
export default function App() {
  ...
  useEffect(() => {
    if (touchupQuery && (page === 'home')) {
        setSearching(true);
        setResults([]);
        setMsg({ type: '', text: '' });
        
        fetch(`${GAS_API_URL}?q=${encodeURIComponent(touchupQuery)}`)
            .then(res => res.json())
            .then(data => {
                if (data.data && data.data.length > 0) {
                    setResults(data.data);
                    setMsg({ type: 'success', text: `找到 ${data.data.length} 筆資料` });
                } else {
  ...
  const handleTouchupSearch = () => {
      if(!searchInput) return alert('請輸入姓名或手機');
      setTouchupQuery(searchInput);
  };
  ...
  if (page === 'home') return (
    <div className="min-h-screen flex flex-col justify-center items-center p-6 bg-[#faf9f6] relative">
       ...
       <Modal title="補色價格查詢" isOpen={!!touchupQuery && (page === 'home')} onClose={() => { setTouchupQuery(''); setResults([]); setMsg({type:'', text:''}); }}>
